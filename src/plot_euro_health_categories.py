...
def plot_health_stackplot(annotation_df, legis_df, output_path, plot_start_year=2000):
    ...
    for cat in HEALTH_CATEGORIES:
        policy_years[cat] = policy_years["Health keyword categories"].str.contains(
    ...
    if 2016 in YEARS:
        ...
        ax.text(X[idx_2016] + 0.2, 0.9 * GLOBAL_Y_MAX, "Paris Agreement in force", va="top", fontsize=10)

    ax.set_ylim(0, GLOBAL_Y_MAX)
    ax.set_ylabel("Legislative documents", fontsize=13)

    ax.set_title(
        "Active Health-Relevant Legislative Documents Over Time in Europe",
        fontsize=13,
        fontweight="bold",
        loc="left"
    )
    ...
    fig.legend(
        ...
        + ["Total health-relevant documents"],
        loc="upper left",
    ...
    fig.text(
        0.01, 0.01,
        f"Note: Analysis shown from {plot_start_year} onward.",
        fontsize=11,
        style="italic"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Health categories stackplot with Family ID-based lifecycle"
    )
